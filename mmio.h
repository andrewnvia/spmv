/*
 *   Matrix Market I/O -- the reading half of the NIST ANSI C library,
 *   see http://math.nist.gov/MatrixMarket for the full thing.
 *
 *   Header-only (every function is `static`) so a program is one .c file:
...
/* Human-readable typecode, e.g. "matrix coordinate real symmetric", for
 * error messages. Returns a pointer into a static buffer. */
static const char *mm_typecode_to_str(MM_typecode matcode)
{
    static char buffer[MM_MAX_LINE_LENGTH];
    const char *types[4];

    types[0] = mm_is_matrix(matcode) ? MM_MTX_STR : "?";
