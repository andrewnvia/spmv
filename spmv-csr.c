/* Sequential CSR SpMV baseline -- PROVIDED.
 *
 * Single-threaded; the kernel both OpenMP programs check their output
 * against. Same problem setup (same seeded values and x), so the numbers
 * compare directly.
 *
 *   ./spmv-csr matrix.mtx [--iters=N]
 */
...
int main(int argc, char** argv)
{
    ...
    /* One offset per row instead of one row index per nonzero saves
